...
def markov_process_step(state, transition_probabilities):
    new_state=np.random.choice(len(transition_probabilities[state]), p=transition_probabilities[:,state])
    return new_state
...
reward_structure[3]=1
reward_structure[8]=1
reward_structure[10]=1
np.random.seed(0)
T=10
states=np.zeros(T, dtype='uint8')
