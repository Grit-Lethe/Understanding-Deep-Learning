...
delta=0.0001
dl_dphi0_est = (ComputeLoss(data[0,:],data[1,:],Model,phi+np.array([[delta],[0]])) - \
                ComputeLoss(data[0,:],data[1,:],Model,phi))/delta
dl_dphi1_est = (ComputeLoss(data[0,:],data[1,:],Model,phi+np.array([[0],[delta]])) - \
                ComputeLoss(data[0,:],data[1,:],Model,phi))/delta
print("Approx gradients: (%3.3f,%3.3f)"%(dl_dphi0_est,dl_dphi1_est))

def LossFunction1D(dist_prop,data,model,phi_start,gradient):
    return ComputeLoss(data[0,:],data[1,:],Model,phi_start+gradient*dist_prop)

def LineSearch(data,model,phi,gradient,thresh=0.00001,max_dist=0.1,max_iter=15.0,verbose=False):
    a=0
    b=0.33*max_dist
    c=0.66*max_dist
    d=1.0*max_dist
...
def GradientDescentStep(phi,data,model):
    gradient=ComputeGradient(data[0,:],data[1,:],phi)
    alpha=LineSearch(data,model,phi,gradient*-1,max_dist=2.0)
...
def GradientDescentStepFixedLearningRate(phi,data,alpha):
    gradient=ComputeGradient(data[0,:],data[1,:],phi)
    phi=phi-alpha*gradient
    return phi
...
DrawModel(data,Model,phi_all[:,0:1],"Initial parameters, Loss = %f"%(loss))
for c_step in range(n_steps):
    phi_all[:,c_step+1:c_step+2] = GradientDescentStepFixedLearningRate(phi_all[:,c_step:c_step+1],data,alpha=0.2)
    if c_step%4==0:
        loss = ComputeLoss(data[0,:], data[1,:], Model, phi_all[:,c_step+1:c_step+2])
        DrawModel(data,Model,phi_all[:,c_step+1], "Iteration %d, loss = %f"%(c_step+1,loss))
DrawLossFunction(ComputeLoss,data,Model,phi_all)

def StochasticGradientDescentStep(phi,data,alpha,batch_size):
    n_data=data.shape[1]
    indices=np.random.permutation(n_data)
    batch_indices=indices[:batch_size]
    gradient=ComputeGradient(data[0,batch_indices],data[1,batch_indices],phi)
    phi=phi-alpha*gradient
    return phi
...
DrawModel(data,Model,phi_all[:,0:1], "Initial parameters, Loss = %f"%(loss))
for c_step in range (n_steps):
    phi_all[:,c_step+1:c_step+2] = StochasticGradientDescentStep(phi_all[:,c_step:c_step+1],data, alpha =0.5, batch_size=5)
    if c_step % 8 == 0:
      loss =  ComputeLoss(data[0,:], data[1,:], Model, phi_all[:,c_step+1:c_step+2])
      DrawModel(data,Model,phi_all[:,c_step+1], "Iteration %d, loss = %f"%(c_step+1,loss))
