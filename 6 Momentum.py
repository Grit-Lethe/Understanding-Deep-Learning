...
for c_step in range(n_steps):
    ...
    gradient=ComputeGradient(data[0,batch_index],data[1,batch_index],phi_all[:,c_step:c_step+1])
...
for c_step in range(n_steps):
    batch_index=np.random.permutation(data.shape[1])[0:batch_size]
    data_in=data[0,batch_index]-alpha*beta*momentum
    gradient=ComputeGradient(data_in,data[1,batch_index],phi_all[:,c_step:c_step+1])
    momentum=beta*momentum+(1-beta)*gradient
    phi_all[:,c_step+1:c_step+2]=phi_all[:,c_step:c_step+1]-alpha*momentum
loss=ComputeLoss(data[0,:],data[1,:],Model,phi_all[:,c_step+1:c_step+2])
DrawModel(data,Model,phi_all[:,c_step+1],'Iteration %d, Loss=%f'%(c_step+1,loss))
